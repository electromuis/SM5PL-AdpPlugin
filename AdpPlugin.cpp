...
std::vector<ButtonMapEntry> AdpPlugin::ButtonMap()
{
	...
	RString deviceList = INPUTMAN->GetDisplayDevicesString();
	int searchIdx = 0;
	do {
		int dIdx = deviceList.find_first_of("(", searchIdx) - searchIdx;
		RString dName = deviceList.substr(searchIdx, dIdx-1);

		int jIdx = deviceList.find_first_of(")", dIdx) - dIdx;
		RString jName = deviceList.substr(dIdx+1, jIdx);
	...
	FOREACH_ENUM(GameController, p)
	{
		...
		{
			RString gbName = scheme->GetGameButtonName(gb);
			if (gbName.size() == 0) {
				continue;
			}
